...
XCODEPROJ="${PROJECT_ROOT}/VibeCockpit.xcodeproj"
...
if [ "$VERBOSE" = true ]; then
    ...
    xcodebuild archive \
        ...
        -destination "generic/platform=macOS,name=Any Mac" \
        CODE_SIGN_IDENTITY="VibeCockpit-CodeSigning" \
...
if [[ "${CI:-}" == "true" ]]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        cat <<EOF
    <enclosure
        url="https://github.com/HZ-CaiBadou/$PROJECT_NAME/releases/download/v$VERSION/$DMG_NAME"
        $SIGN_OUTPUT
