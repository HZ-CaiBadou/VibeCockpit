...
class MenuBarIconRenderer {
    ...
    /// 菜单栏品牌图标尺寸
    private let providerBrandIconSize: CGFloat = 18
    /// 菜单栏组合画布高度
    private let metricIconSize: CGFloat = 18
    /// 菜单栏百分比环尺寸
    private let metricCircleIconSize: CGFloat = 20
    ...
    private func createCircleImage(percentage: Double, displayPercentage: Double? = nil, size: NSSize, useSevenDayColor: Bool = false, colorOverride: NSColor? = nil, useDashedStyle: Bool = false, button: NSStatusBarButton?, removeBackground: Bool = false) -> NSImage {
        let image = NSImage(size: size)
        image.lockFocus()

        let center = NSPoint(x: size.width / 2, y: size.height / 2)
        let radius = min(size.width, size.height) / 2 - CGFloat(UsageDisplayMath.menuBarCircleRadiusInset)
        let usedPercentage = UsageDisplayMath.clampedPercentage(percentage)
        let drawnPercentage = UsageDisplayMath.clampedPercentage(displayPercentage ?? percentage)
        ...
        backgroundPath.lineWidth = CGFloat(UsageDisplayMath.menuBarCircleBackgroundLineWidth)
        ...
        if let override = colorOverride {
            ...
            color = useSevenDayColor ? UsageColorScheme.sevenDayColorAdaptive(usedPercentage, for: button) : UsageColorScheme.fiveHourColorAdaptive(usedPercentage, for: button)
        }
        color.setStroke()

        let progressPath = NSBezierPath()
        let lineWidth = CGFloat(UsageDisplayMath.menuBarCircleProgressLineWidth)

        // 计算进度角度
        let baseAngle = CGFloat(drawnPercentage) / 100.0 * 360
        ...
        if drawnPercentage >= 100 {
            ...
            progressAngle = baseAngle - capAngle * min(1.0, CGFloat(drawnPercentage / 50.0))
        ...
        progressPath.lineCapStyle = drawnPercentage >= 100 ? .butt : .round
        progressPath.stroke()

        let fontSize = size.width * CGFloat(UsageDisplayMath.menuBarMetricFontScale(isThreeDigit: drawnPercentage >= 100))
        let font = NSFont.systemFont(ofSize: fontSize, weight: .bold)
        let text = "\(Int(drawnPercentage))"
        let paragraphStyle = NSMutableParagraphStyle()
        paragraphStyle.alignment = .center

        let attrs: [NSAttributedString.Key: Any] = [
            .font: font,
            .foregroundColor: metricTextColor(isMonochrome: false),
    ...
    private func createCircleTemplateImage(percentage: Double, displayPercentage: Double? = nil, size: NSSize, useSevenDayStyle: Bool = false, button: NSStatusBarButton? = nil, removeBackground: Bool = false) -> NSImage {
        let image = NSImage(size: size)
        image.lockFocus()
        ...
        let attrs: [NSAttributedString.Key: Any] = [
            .font: font,
            .foregroundColor: metricTextColor(isMonochrome: true),
            .paragraphStyle: paragraphStyle
        ]
        let textSize = text.size(withAttributes: attrs)
    ...
    private func metricTextColor(isMonochrome: Bool) -> NSColor {
        UsageDisplayMath.menuBarMetricTextTone(isMonochrome: isMonochrome) == .light ? .white : .black
    }
    ...
    func createIconForType(
        ...
        case .fiveHour:
            ...
            let displayPercentage = menuBarDisplayPercentage(for: percentage)
            if isMonochrome {
                ...
                return createCircleImage(percentage: percentage, displayPercentage: displayPercentage, size: NSSize(width: metricCircleIconSize, height: metricCircleIconSize), button: button, removeBackground: removeBackground)
            }
        ...
        case .sevenDay:
            ...
            if isMonochrome {
                ...
                return createCircleImage(percentage: percentage, displayPercentage: displayPercentage, size: NSSize(width: metricCircleIconSize, height: metricCircleIconSize), useSevenDayColor: true, button: button, removeBackground: removeBackground)
            }

        case .opusWeekly:
            let percentage = data.opus?.percentage ?? (showPlaceholder ? 0 : nil)
            guard let percentage = percentage else { return nil }
            return ShapeIconRenderer.createVerticalRectangleIcon(percentage: percentage, displayPercentage: menuBarDisplayPercentage(for: percentage), isMonochrome: isMonochrome, button: button, removeBackground: removeBackground)

        case .sonnetWeekly:
            let percentage = data.sonnet?.percentage ?? (showPlaceholder ? 0 : nil)
            guard let percentage = percentage else { return nil }
            return ShapeIconRenderer.createHorizontalRectangleIcon(percentage: percentage, displayPercentage: menuBarDisplayPercentage(for: percentage), isMonochrome: isMonochrome, button: button, removeBackground: removeBackground)
        ...
        case .extraUsage:
            ...
            guard let percentage = percentage else { return nil }
            return ShapeIconRenderer.createHexagonIcon(percentage: percentage, displayPercentage: menuBarDisplayPercentage(for: percentage), isMonochrome: isMonochrome, button: button, removeBackground: removeBackground)
    ...
    func createCodexIcon(
        ...
        case .codexExtraUsage:
            ...
            return ShapeIconRenderer.createHexagonIcon(percentage: percentage, displayPercentage: displayPercentage, isMonochrome: isMonochrome, button: button, removeBackground: removeBackground, colorOverride: color)
    ...
    private func menuBarDisplayPercentage(for usedPercentage: Double) -> Double {
        UsageDisplayMath.displayPercentage(usedPercentage: usedPercentage, displayMode: .remaining)
    }
