...
class ShapeIconRenderer {
    ...
    static func metricTextColor(isMonochrome: Bool) -> NSColor {
        UsageDisplayMath.menuBarMetricTextTone(isMonochrome: isMonochrome) == .light ? .white : .black
    }

    static func metricFontSize(for drawnPercentage: Double) -> CGFloat {
        18 * CGFloat(UsageDisplayMath.menuBarMetricFontScale(isThreeDigit: drawnPercentage >= 100))
    }
    ...
    static func drawRoundedSquareWithPercentage(in rect: NSRect, percentage: Double, displayPercentage: Double? = nil, isMonochrome: Bool, button: NSStatusBarButton?, removeBackground: Bool = false) {
        ...
        if drawnPercentage > 0 {
            ...
            let baseProgressLength = perimeter * CGFloat(drawnPercentage / 100.0)
            let progressLength = drawnPercentage >= 100 ? baseProgressLength : (baseProgressLength - progressWidth * min(1.0, CGFloat(drawnPercentage / 50.0)))
            ...
            let phase: CGFloat = drawnPercentage >= 100 ? 0 : -progressWidth / 2
            ...
            if isMonochrome {
                let opacity = monochromeOpacity(for: usedPercentage)
                NSColor.controlTextColor.withAlphaComponent(opacity).setStroke()
            } else {
                UsageColorScheme.opusWeeklyColorAdaptive(usedPercentage, for: button).setStroke()
            }
            progressPath.stroke()
        }

        // 3. 绘制百分比文字
        let percentageText = "\(Int(drawnPercentage))"
        let percentageFontSize = metricFontSize(for: drawnPercentage)
        let attributes: [NSAttributedString.Key: Any] = [
            .font: NSFont.systemFont(ofSize: percentageFontSize, weight: drawnPercentage >= 100 ? .bold : .semibold),
            .foregroundColor: metricTextColor(isMonochrome: isMonochrome)
        ]
    ...
    ///   - button: 状态栏按钮（用于获取颜色）
    ///   - removeBackground: 是否移除背景填充
    static func drawDiamondWithPercentage(in rect: NSRect, percentage: Double, displayPercentage: Double? = nil, isMonochrome: Bool, button: NSStatusBarButton?, removeBackground: Bool = false) {
        // 完全复制Opus的参数设置
        let cornerRadius: CGFloat = 3.0
        let borderWidth: CGFloat = 1.5
        let progressWidth: CGFloat = 2.5  // 进度线条加粗
        ...
        if drawnPercentage > 0 {
            ...
            if isMonochrome {
                ...
                UsageColorScheme.sonnetWeeklyColorAdaptive(usedPercentage, for: button).setStroke()
            }
            progressPath.stroke()
        }
    ...
    ///   - button: 状态栏按钮（用于获取颜色）
    ///   - removeBackground: 是否移除背景填充
    static func drawHexagonWithPercentage(center: NSPoint, size: CGFloat, percentage: Double, displayPercentage: Double? = nil, isMonochrome: Bool, button: NSStatusBarButton?, removeBackground: Bool = false, colorOverride: NSColor? = nil) {
        let radius = size / 2
        let borderWidth: CGFloat = 1.5
        let progressWidth: CGFloat = 2.5  // 进度线条加粗
        let usedPercentage = UsageDisplayMath.clampedPercentage(percentage)
        ...
        if drawnPercentage > 0 {
            ...
            // 100%时使用平头让图形完美闭合，其他进度使用圆头
            progressHexagon.lineCapStyle = drawnPercentage >= 100 ? .butt : .round
            ...
            if isMonochrome {
                ...
                UsageColorScheme.extraUsageColorAdaptive(usedPercentage, for: button).setStroke()
            }
    ...
    ///   - removeBackground: 是否移除背景填充
    /// - Returns: 图标图像 (18×18)
    static func createVerticalRectangleIcon(percentage: Double, displayPercentage: Double? = nil, isMonochrome: Bool, button: NSStatusBarButton?, removeBackground: Bool = false) -> NSImage {
        let size = NSSize(width: 18, height: 18)
        let image = NSImage(size: size)
        image.lockFocus()

        let rect = NSRect(x: 0, y: 0, width: size.width, height: size.height).insetBy(dx: 2, dy: 2)
        drawRoundedSquareWithPercentage(in: rect, percentage: percentage, displayPercentage: displayPercentage, isMonochrome: isMonochrome, button: button, removeBackground: removeBackground)
    ...
    ///   - removeBackground: 是否移除背景填充
    /// - Returns: 图标图像 (18×18)
    static func createHorizontalRectangleIcon(percentage: Double, displayPercentage: Double? = nil, isMonochrome: Bool, button: NSStatusBarButton?, removeBackground: Bool = false) -> NSImage {
        let size = NSSize(width: 18, height: 18)
        let image = NSImage(size: size)
        image.lockFocus()

        let rect = NSRect(x: 0, y: 0, width: size.width, height: size.height).insetBy(dx: 2, dy: 2)
        drawDiamondWithPercentage(in: rect, percentage: percentage, displayPercentage: displayPercentage, isMonochrome: isMonochrome, button: button, removeBackground: removeBackground)
    ...
    /// - Returns: 图标图像 (18×18)
    static func createHexagonIcon(percentage: Double, displayPercentage: Double? = nil, isMonochrome: Bool, button: NSStatusBarButton?, removeBackground: Bool = false, colorOverride: NSColor? = nil) -> NSImage {
        let size = NSSize(width: 18, height: 18)
        let image = NSImage(size: size)
        image.lockFocus()

        let center = NSPoint(x: size.width / 2, y: size.height / 2)
        drawHexagonWithPercentage(center: center, size: 16, percentage: percentage, displayPercentage: displayPercentage, isMonochrome: isMonochrome, button: button, removeBackground: removeBackground, colorOverride: colorOverride)
