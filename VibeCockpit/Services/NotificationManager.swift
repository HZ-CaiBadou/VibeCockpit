...
class NotificationManager {
    ...
    func sendCodexWakeupFailedNotification(message: String) {
        let content = UNMutableNotificationContent()
        content.title = L.UsageNotification.codexWakeupFailedTitle
        content.body = L.UsageNotification.codexWakeupFailedBody(message)
        content.sound = .default

        let request = UNNotificationRequest(
            identifier: "codex_wakeup_failed_\(Date().timeIntervalSince1970)",
            content: content,
            trigger: nil
        )
        ...
        UNUserNotificationCenter.current().add(request) { error in
            if let error = error {
                Logger.menuBar.error("发送 Codex 唤醒失败通知失败: \(error.localizedDescription)")
            }
        }

        Logger.menuBar.info("已发送 Codex 唤醒失败通知")
    }

    func sendCodexWakeupDisabledNotification() {
        let content = UNMutableNotificationContent()
        content.title = L.UsageNotification.codexWakeupDisabledTitle
        content.body = L.UsageNotification.codexWakeupDisabledBody
        content.sound = .default

        let request = UNNotificationRequest(
            identifier: "codex_wakeup_disabled_\(Date().timeIntervalSince1970)",
            content: content,
            trigger: nil
        )
        ...
        UNUserNotificationCenter.current().add(request) { error in
            if let error = error {
                Logger.menuBar.error("发送 Codex 自动唤醒停用通知失败: \(error.localizedDescription)")
            }
        }

        Logger.menuBar.info("已发送 Codex 自动唤醒停用通知")
    }
